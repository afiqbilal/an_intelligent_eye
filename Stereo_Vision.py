...
for i in range(0,56):   # Put the amount of pictures you have taken for the calibration inbetween range(0,?) wenn starting from the image number 0
...
#   Right Side
#it returns the camera matrix, distortion coefficient, rotation & translation vector
...
#matrix left stereo, distortion left stereo, matrix right stereo
retS, MLS, dLS, MRS, dRS, R, T, E, F= cv2.stereoCalibrate(objpoints,
                                                          ...
                                                          criteria_stereo,
                                                          flags)
#R is Output rotation matrix b/w first and second camera coordinate system
#T is Output Translation vector, E is essential matrix, F is fundamental matrix
# StereoRectify function
rectify_scale= 0 # if 0 image cropped, if 1 image not cropped
RL, RR, PL, PR, Q, roiL, roiR= cv2.stereoRectify(MLS, dLS, MRS, dRS,
                                                 ChessImaR.shape[::-1], R, T,
                                                 rectify_scale,(0,0))  # last paramater is alpha, if 0= cropped, if 1= not cropped

#RL is Rectification Transform(Rotation matrix for first camera), PL is projection matrix
#Q is Output 4x4 disparity to depth mapping matrix,roiL is optional output rectangle
# initUndistortRectifyMap function
Left_Stereo_Map= cv2.initUndistortRectifyMap(MLS, dLS, RL, PL,
                                             ChessImaR.shape[::-1], cv2.CV_16SC2)   # cv2.CV_16SC2 this format enables the programme to work faster
...
#P1 P2 controlling disparity smoothness
# Used for the filtered image
stereoR=cv2.ximgproc.createRightMatcher(stereo) # Create another stereo for right
...
CamR= cv2.VideoCapture(0)   # When 0 Right Cam
CamL= cv2.VideoCapture(1)   # When 1 Left Cam
...
#################################################################################################################    
    ...
    # Draw Red lines
    for line in range(0, int(Right_nice.shape[0]/20)): # Draw the Lines on the images, number of lines is defined by the image Size/20
        Left_nice[line*20,:]= (0,0,255)
        Right_nice[line*20,:]= (0,0,255)

    for line in range(0, int(frameR.shape[0]/20)): # Draw the Lines on the images, number of lines is defined by the image Size/20
...
##    dispR= cv2.resize(disp,None,fx=0.7, fy=0.7, interpolation = cv2.INTER_AREA)
    ...
    closing= cv2.morphologyEx(disp,cv2.MORPH_CLOSE, kernel) # Applying a morphological filter for Removal of noise 

    # Colors map
    dispc= (closing-closing.min())*255
    dispC= dispc.astype(np.uint8)                                   # Convert the type of the matrix from float32 to uint8 to show the results with the function cv2.imshow()
